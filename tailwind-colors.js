// Tailwind CSS v3 Color Palette Reference
// Used for finding closest Tailwind color match
...
function findClosestTailwind(hex) {
    ...
    for (const [colorName, shades] of Object.entries(TAILWIND_COLORS)) {
        for (const [shade, shadeHex] of Object.entries(shades)) {
            const shadeRgb = hexToRgbArray(shadeHex);
            if (!shadeRgb) continue;

            const distance = colorDistance(rgb, shadeRgb);
            if (distance < closest.distance) {
                closest = {
                    name: shade === 'DEFAULT' ? colorName : `${colorName}-${shade}`,
                    hex: shadeHex,
                    distance
                };
            }
...
if (typeof window !== 'undefined') {
    window.TailwindColors = {
        ...
        findClosest: findClosestTailwind
    };
}
